...
import { IdentifierPairedExpression } from '@microsoft/powerquery-parser/lib/powerquery-parser/language/ast/ast'; 
...
{ 
  return node.kind == Ast.NodeKind.IdentifierPairedExpression  
    ...
    || node.kind == Ast.NodeKind.FieldSpecification; 
} 
 
function getKeyLength(node: Ast.TNode) 
{ 
  switch(node.kind) 
  { 
    case Ast.NodeKind.IdentifierPairedExpression: 
      return node.key.literal.length; 
    case Ast.NodeKind.GeneralizedIdentifierPairedExpression: 
      return node.key.literal.length; 
    case Ast.NodeKind.GeneralizedIdentifierPairedAnyLiteral: 
      return node.key.literal.length; 
    case Ast.NodeKind.FieldSpecification: 
      return node.name.literal.length  
        + (node.optionalConstant  
            ? (node.optionalConstant.constantKind.length + 1)  
            : 0) 
    default: 
      return null; 
  } 
...
{ 
  ...
  else if(parent.kind == Ast.NodeKind.RecordExpression  
    || parent.kind == Ast.NodeKind.RecordLiteral  
    || parent.kind == Ast.NodeKind.RecordType 
    || parent.kind == Ast.NodeKind.FieldSpecificationList 
  ) 
  { 
...
{ 
  ...
  { 
    ...
    let keyLength = (this as Ast.IArrayWrapper<Ast.ICsv<Ast.TNode>>).elements 
      .map(e => getKeyLength(e.node)) 
      .reduce((c,v) => c < v ? v : c, 0); 
